...
export class UsersListComponent implements OnInit {
  ...
  public model: FriendCardInfo;

  search = (text$: Observable<string>) =>
    text$.pipe(
      debounceTime(200),
      map(term => term === '' ? []
        : this.friends.filter(v => v.login.toLowerCase().indexOf(term.toLowerCase()) > -1).slice(0, 10))
    )
  ...
  ngOnInit() {
    this.getAllUsersWithoutLoggedInUser();
  }



  getAllUsersWithoutLoggedInUser(){
    this.friendsService.getAllUsersWithoutLoggedInUser()
    .subscribe( data => {
      this.friends  = JSON.parse(JSON.stringify( data ));
      this.logins = [];
      for(let i =0; i<this.friends.length;i++){
        this.logins[i] = this.friends[i].login;   
      }    
   }, error => {
     this.alertify.error("Error " + error.status + " - Couldn't load users info");
   });
  }

  goToUser(){
    this.router.navigate(['/searchUser/userProfile/', this.model.login]);
